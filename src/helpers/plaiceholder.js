...
var arrayChunk = function arrayChunk(arr, size) {
  return arr.length > size
    ? [arr.slice(0, size)].concat(arrayChunk(arr.slice(size), size))
    : [arr];
};
...
var getCSS = function getCSS(_ref4) {
  ...
  var linearGradients = pixels.map(function (row) {
    ...
    var gradient = rowPixels
      .map(function (pixel, i) {
  ...
  var backgroundPosition = linearGradients
    .map(function (_, i) {
      return i === 0
        ? "0 0 "
        : "0 " + (i / (linearGradients.length - 1)) * 100 + "%";
    })
    .join(",");
...
var getPlaiceholder = function getPlaiceholder(src, _temp) {
  ...
  try {
    ...
    return Promise.resolve(
      ...
      var pipelineStage2 =
        removeAlpha === false ? pipelineStage1 : pipelineStage1.removeAlpha(); // autoOrientation
